...
def page_comparison_details():
    ...
    col1, col2 = st.columns([1, 3])
    with col1:
        if st.button("← Zurück zu Vergleichen", key="back_to_compare_details", use_container_width=True):
            st.session_state.pop('page', None)
...
def tab_info():
    ...
    **ML Training Service** ist ein Machine-Learning-Service für Kryptowährungs-Datenanalyse.
    ...
    PostgreSQL Datenbank
            ...
            ├─ coin_streams (ATH-Tracking: ath_price_sol, ath_timestamp)
    ...
    Training Engine (app/training/engine.py)
            ├─ Lädt Daten aus coin_metrics (mit ATH-JOIN zu coin_streams)
            ├─ Feature-Engineering (optional, inkl. ATH-Features)
    ...
    **Prozess:**
    ...
    5. **Feature-Engineering:** Optional werden ~70 zusätzliche Features erstellt (inkl. ATH-Features)
    ...
    **🆕 ATH-Tracking (All-Time High):**
    - `ath_price_sol` - All-Time High Preis (aus `coin_streams`)
    - `ath_timestamp` - Timestamp des letzten ATH-Updates
    - `price_vs_ath_pct` - Wie weit ist aktueller Preis vom ATH entfernt? (negativ = unter ATH)
    ...
    **Basis-Metriken:**
    - `ath_price_sol` - All-Time High Preis (wird live getrackt)
    - `ath_timestamp` - Timestamp des letzten ATH-Updates
    - `price_vs_ath_pct` - Wie weit ist aktueller Preis vom ATH entfernt? (negativ = unter ATH)
    - `minutes_since_ath` - Wie lange ist es her, dass ATH erreicht wurde?
    
    **ATH-basierte Features (Feature-Engineering):**
    - `ath_distance_pct` - Abstand vom ATH in Prozent
    - `is_near_ath` - Innerhalb 5% vom ATH? (0/1)
    - `is_at_ath` - Innerhalb 0.1% vom ATH? (0/1)
    - `ath_breakout` - Neuer ATH erreicht? (0/1)
    - `ath_breakout_volume` - Volumen bei Breakout
    - `ath_approach_{window}` - Nähert sich dem ATH? (Trend)
    - `ath_breakout_count_{window}` - Anzahl Breakouts im Fenster
    - `ath_age_hours` - Alter des ATH in Stunden
    - `ath_is_recent` - ATH innerhalb 1 Stunde? (0/1)
    - `ath_is_old` - ATH älter als 24 Stunden? (0/1)
    - Und viele mehr...
